...
By default, PDFQuery will cache the parse tree for each PDF to the /tmp/ directory. To use a different directory:

    from pdfquery.cache import FileCache
    pdfquery.PDFQuery("tests/sample.pdf", parse_tree_cacher=FileCache("/foo/"))

To disable caching, use parse_tree_cacher=None.
...
*   parse_tree_cacher: an object that knows how to save and load results of parsing a given page range from a given PDF.
    Pass in FileCache('/foo/') to save caches to a different directory, or None to disable caching.
