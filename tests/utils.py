...
class BaseTestCase(unittest.TestCase):
    def assertValidOutput(self, pdf, output_name):
        ...
        try:
            ...
            raise self.failureException(
                "XML conversion of sample pdf has changed! Compare %s to %s"
    ...
    def xml_strings_equal(self, s1, s2, ignore_attribs=IGNORE_ATTRIBS):
        ...
        def elements_equal(e1, e2):
            if e1.tag != e2.tag:
                raise self.failureException("Mismatched tags", e1, e2)
            if e1.text != e2.text:
                raise self.failureException("Mismatched text", e1, e2)
            if e1.tail != e2.tail:
                raise self.failureException("Mismatched tail", e1, e2)
            if set(e1.attrib) - ignore_attribs != set(e2.attrib) - ignore_attribs:
                raise self.failureException(
                    "Mismatched attributes %s and %s" % (e1.attrib, e2.attrib), e1, e2
                )
            if len(e1) != len(e2):
                raise self.failureException("Mismatched children", e1, e2)
